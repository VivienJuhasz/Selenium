...
public class Feladat28Chrome {
    ...
    @Test
    public void testFeladat28Chrome() throws InterruptedException {
        driver.get(baseURL);
        Thread.sleep(3000);

        driver.findElement(By.linkText("CHAPTER14")).click();
        Thread.sleep(3000);
        WebElement messageWebElement = driver.findElement(By.id("message"));
        System.out.println(messageWebElement.getCssValue("backgroundColor"));
        assertEquals("rgba(0, 0, 255, 1)", messageWebElement.getCssValue("backgroundColor"));

        Actions builder = new Actions(driver);
        builder.doubleClick(messageWebElement)
                .build()
                .perform();
        Thread.sleep(2000);
        System.out.println(messageWebElement.getCssValue("backgroundColor"));
        assertEquals("rgba(255, 255, 0, 1)", messageWebElement.getCssValue("backgroundColor"));
